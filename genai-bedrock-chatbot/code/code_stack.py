...
VALORANT_ESPORTS_DESTINATION_PREFIX = "source_data"
...
class CodeStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        kendra_bucket, val_bucket = self.create_data_source_bucket(kms_key)
        kendra_index = self.create_kendra_index(kendra_bucket, kms_key)
        self.upload_files_to_s3(kendra_bucket, val_bucket, kms_key)
        glue_database, _ = self.create_glue_database(val_bucket, kms_key)
    ...
    def create_data_source_bucket(self, kms_key):
        ...
        # creating kendra source bucket
        val_bucket = s3.Bucket(
            self,
            "ValorantPagesBucket",
            bucket_name=f"{Aws.STACK_NAME}-valorant-pages-{Aws.ACCOUNT_ID}",
            versioned=True,
            removal_policy=RemovalPolicy.DESTROY,
            encryption=s3.BucketEncryption.KMS,
            encryption_key=kms_key,
        ...
        CfnOutput(self, "PagesBucket", value=val_bucket.bucket_name)
        return kendra_bucket, val_bucket
    ...
    def upload_files_to_s3(self, kendra_bucket, val_bucket, kms_key):
        ...
        s3deploy.BucketDeployment(
            self,
            "ValorantEsportsPagesDeployment",
            sources=[
                s3deploy.Source.asset(
                    path.join(os.getcwd(), ASSETS_FOLDER_NAME, "valorant_source")
                )
            ],
            destination_bucket=val_bucket,
            retain_on_delete=False,
            destination_key_prefix=VALORANT_ESPORTS_DESTINATION_PREFIX,
        )
        return

    def create_glue_database(self, val_bucket, kms_key):
        ...
        val_bucket.grant_read(glue_role)
        ...
        glue_database = glue.CfnDatabase(
            self,
            "ValorantEsportDatabase",
            catalog_id=Aws.ACCOUNT_ID,
            database_input=glue.CfnDatabase.DatabaseInputProperty(
                name=f"{Aws.STACK_NAME}-esports-db"
            ),
        )
        ...
        cfn_crawler = glue.CfnCrawler(
            self,
            "ValorantEsportsCrawler",
            role=glue_role.role_name,
            crawler_security_configuration=cfn_security_configuration.name,
            targets=glue.CfnCrawler.TargetsProperty(
                s3_targets=[
                    # glue.CfnCrawler.S3TargetProperty(
                    #     path=f"s3://{val_bucket.bucket_name}/{VALORANT_ESPORTS_DESTINATION_PREFIX}/game_changers_2024_fixture/",
                    # ),
                    glue.CfnCrawler.S3TargetProperty(
                        path=f"s3://{val_bucket.bucket_name}/{VALORANT_ESPORTS_DESTINATION_PREFIX}/game_changers_2024_gamedata/",
                    ),
                ]
            ),
            database_name=glue_database.ref,
            description="Crawler job for valorant esports",
            name=f"{Aws.STACK_NAME}-valorant-esports-crawler",
        )
        NagSuppressions.add_resource_suppressions(
            cfn_crawler,
            suppressions=[
    ...
    def create_lambda_function(
        ...
        lambda_function = lambda_.Function(
            ...
            environment={
                ...
                "PRICING_DATA_SOURCE_BUCKET_NAME": val_bucket.bucket_name,
                "VALORANT_ESPORTS_DATABASE": glue_database.ref,
        ...
        val_bucket.grant_read_write(lambda_role)
